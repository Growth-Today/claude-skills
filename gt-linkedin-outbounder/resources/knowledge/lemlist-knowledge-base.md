...
**Source:** Growth Today's internal "Sequencing Tool Recommendations" SOP (the same source of truth the `gt-cold-email-writer` skill uses). This is our own tested stance, not a vendor claim.
...
| Skill / file | Why |
|---|---|
| `gt-linkedin-outbounder` | Main skill; the multi-channel coordination lives here |
| `gt-cold-email-writer` | Owns the email side of a multi-channel motion; the two skills share the sequencing-tool SOP |
| `gt-clay-builder` | Clay pushes enriched leads and custom variables into the sequence |
| `gt-buying-signal-sourcer` | Signals decide which channel opens and with what angle |
