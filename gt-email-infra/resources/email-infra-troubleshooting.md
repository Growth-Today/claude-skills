...
Diagnosis and recovery protocols for common email infrastructure problems. For healthy metrics reference → see [email-infra-reference.md](email-infra-reference.md).
...
**Note:** Especially common with GoDaddy, which sometimes auto-creates SPF records.
...
3. Mailboxes properly warmed (minimum 14 days)?
4. Sending volume within limits? (see [email-infra-reference.md](email-infra-reference.md))
5. Email content spammy? (trigger words, too many links, HTML)
6. On any blacklists? (MXToolbox check)
7. Custom tracking domain enabled?
8. Profile pictures on all mailboxes?

### Bounce Rate Spike (Above 3%)
...
**To pinpoint *where* the bounce comes from** — read the SMTP code, don't just bucket it. A wave of hard `5.x.x` / "does not exist" bounces points at list/verification/data accuracy; soft `4.x.x` points at temporary or infra issues; `5.7.1` / policy / blocked points at corporate filtering or reputation. For the full diagnosis workflow (categorizing bounces, soft vs hard codes, root-cause → which layer to fix), see [blacklist-bounce-audit.md](blacklist-bounce-audit.md).
...
4. Any external factors? (holidays, industry events — see [email-infra-reference.md](email-infra-reference.md))
...
1. Re-watch the setup videos in [email-infra-step-by-step.md](email-infra-step-by-step.md) — most issues are missed steps
2. Contact registrar support — they can verify records are saved
3. MXToolbox "Check DNS" feature — shows exactly what's missing
4. Run mail-tester.com — full deliverability score with diagnosis
