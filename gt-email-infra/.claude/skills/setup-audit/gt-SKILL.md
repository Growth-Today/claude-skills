...
description: "Audit a live sequencer workspace to verify inboxes are set up correctly against the Growth Today standard. Use for pulling live config from Instantly / EmailBison / Smartlead / Lemlist and reporting per-item PASS / WARN / FAIL across connection, warmup, sending limits, DNS/auth, tracking, Unibox toggles and cross-sequencer hygiene. The 21-dimension table is executable: each row names the exact MCP call and field. Triggers on setup audit, run the audit, audit my client, is this workspace set up right, check warmup settings, Unibox settings, pre-launch setup check. Do NOT use to diagnose bounces/blacklists (use blacklist-bounce-audit), read ongoing inbox health (use dashboard-reading), or do first-time setup (use the platform setup sub-skills)."
---
