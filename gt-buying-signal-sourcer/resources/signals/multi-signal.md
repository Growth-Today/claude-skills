...
- Read `{SKILL_BASE}/resources/core/signal-scoring.md` for the complete scoring framework (weights, recency, thresholds, SLAs, plays)
- Read `{SKILL_BASE}/resources/plays/gtm-plays.md` for 11 executable GTM plays and multi-channel coordination
- Read `{SKILL_BASE}/resources/detection/signal-detection-tools.md` for 30-trigger quick reference with detection tools, timing windows, Clay credit costs, signal freshness rules (when signals expire), reliability tiers, and signal sources by data party (1st/2nd/3rd)
