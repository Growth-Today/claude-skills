...
## Apollo: Input Yes, Intent No

Apollo enrichment and its people/company search are a solid **waterfall input** - Clay's waterfall
email lands ~86-90% coverage vs Apollo single-source at ~62-68% (GT's own assessment), so seed with
Apollo and let the waterfall finish.

**Do not** rely on Apollo for buying signals: Apollo **intent data is NOT exposed** through Clay's
Apollo integration. Source intent from Bombora, Demandbase, or Warmly instead.
